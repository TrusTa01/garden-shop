...
const AllProductsPage = ({ showNotification }) => {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [priceFrom, priceTo, discountedOnly, sortOption]);
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    // 2. Фильтрация по товарам со скидкой
    if (discountedOnly) {
    ...
    return result;
  }, [products, priceFrom, priceTo, discountedOnly, sortOption]);
  ...
  const handlePageChange = (pageNumber) => {
    if (pageNumber > 0 && pageNumber <= totalPages) {
      ...
      // Прокрутка к началу списка продуктов после смены страницы
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };

  if (productsStatus === 'loading') {
    return <p className={styles.loadingMessage}>Loading all products...</p>;
  }
  ...
  return (
    <div className={styles.allProductsPage}>
      ...
      <ProductFilters
        ...
        hideDiscountedCheckbox={false} // Показываем чекбокс на этой странице
      />
